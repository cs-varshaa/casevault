...
class App {
  ...
  init() {
    ...
    // Global custom navigation events
  ...
  bindCurrentViewEvents() {
    const onNav = (view, reRender, params) => {
      ...
      else this.navigateTo(view, params);
    };
  ...
  handleGlobalSearch(query) {
    ...
    // Check if user is looking for a case
    const matchedCase = State.cases.find(c => c.id.toLowerCase().includes(q) || c.name.toLowerCase().includes(q));
    // Check if user is looking for a document
    const matchedDoc = State.documents.find(d => d.name.toLowerCase().includes(q) || d.id.toLowerCase().includes(q) || d.sha256.toLowerCase().includes(q));

    if (matchedCase && this.currentView !== 'cases') {
      this.navigateTo('cases', { caseId: matchedCase.id });
      Toast.info('Search Match', `Found Case: ${matchedCase.id}`);
    } else if (matchedDoc && this.currentView !== 'documents') {
      this.navigateTo('documents');
      Toast.info('Search Match', `Found Document: ${matchedDoc.name}`);
    }
  }
}
